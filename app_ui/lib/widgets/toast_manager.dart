  ...
  void removeToast(String id) {
    ...
    if (index != -1) {
      ...
      _listKey.currentState?.removeItem(
        index,
        (context, animation) =>
            _buildToastItem(removed, animation, isRemoving: true),
        duration: const Duration(milliseconds: 300),
      );
    }
...
class _AnimatedToast extends StatefulWidget {
  ...
  const _AnimatedToast({required this.toast, required this.onDismiss});
...
class _AnimatedToastState extends State<_AnimatedToast>
  ...
  void _onDragEnd(DragEndDetails details) {
    ...
    if (_dragOffset.abs() > 100 || velocity.abs() > 800) {
      ...
      _dragAnimation = Tween<double>(begin: _dragOffset, end: targetOffset)
          .animate(
